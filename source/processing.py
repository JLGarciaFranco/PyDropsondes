        ...
        for sampindex,periodskey in enumerate(sampleperiods):
            ...
            for filename in filelist:
                ...
                # This section (not used unless indicated by user) is made for the raw datasets or dropsondes downloaded individually.
                    ...
                    v=ur*np.sin(np.pi*udir/180)
                    u=ur*np.cos(np.pi*udir/180)
                ...
                pressure=clean2(cleanu(cleanp(clean1(P))))
                ...
                for j,longi in enumerate(lon):
                    ...
                    f.write(str(np.around(r,3))+'\t'+str(np.around(theta,4))+'\t'+str(np.around(u_speed[j],3))+'\t'+str(np.around(v_speed[j],3))+'\t'+str(np.around(w_speed[j],4))+'\t'
