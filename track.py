...
def readtrack(filename,basin):
    f=open(filename,'r')
    a=f.readlines()
    dicc={'Storm Name':[],'Datetime':[],'Longitude':[],'Latitude':[],'pressure':[],'windspeed':[],'Intensity':[],'Basin':[],'Year':[]}
    f.close()
    ii=0
    line=a[ii]
    ...
    while ii<len(a)-1:
        ...
        while len(line.split(','))>5 and ii<len(a)-1:
            ...
            newlist=[stormname,dt,strings[5],strings[4],int(strings[7]),int(strings[6]),strings[3],basin,dt.year]
            ii+=1
            line=a[ii]
